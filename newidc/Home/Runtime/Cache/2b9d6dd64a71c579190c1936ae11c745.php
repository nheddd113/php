...
<html lang="en">
                    ...
                    <div class="page-content">
                        <div class="page-header">
                                ...
                                <div class="pull-right widget-toolbar">
                                    <a href="javascript:void(0)"  data-rel="tooltip" data-placement="top" title="增加机柜" data-target="addCup" >
                        ...
                        <div class="row">
                            <div class="page-header">
                                <h1 class="center">机柜列表</h1>
                            </div>
                            <table id="dynamic-table"  class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        <th class="col-sm-1">序号</th>
                                        <th class="col-sm-1">机柜名称</th>
                                        <th class="col-sm-2">所属机房</th>
                                        <th class="col-sm-1">机位总数</th>
                                ...
                                <tbody>
                                    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                        ...
                                        <td><?php echo ($vo["cupname"]); ?></td>
                                        <td><?php echo ($vo["houname"]); ?></td>
                                        <td><?php echo ($vo["seatnum"]); ?></td>
                                        <td><?php echo ($vo["createtime"]); ?></td>
                                        <td><?php echo ($vo["remark"]); ?></td>
                                        <td>
                                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                <a data-rel="tooltip" data-placement="top" cup="<?php echo ($vo["id"]); ?>" data-target="modifyhouse" title="修改" class="green" href="javascript:void(0)">
                                                    <i class="icon-pencil bigger-130"></i>
                                                </a>
                                                <a data-rel="tooltip" data-placement="top" data-target="deleteHouse" url="<?php echo U('Public/deletecup');?>" sid="<?php echo ($vo["id"]); ?>" title="删除" class="red" href="javascript:void(0);">
                                                    <i class="icon-trash bigger-130"></i>
                                                </a>
                                            </div>
    ...
    <div class="modal fade" id="addCup" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true">
        <div class="modal-dialog" style="width:50%">
             <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h2 class="modal-title">新建机柜</h2>
                </div>
                <div class="modal-body">
                    <form class="form-horizontal" id="createCup" role="form" onsubmit="return false" action="<?php echo U('Create/amendCupHandle');?>" method="POST">
                        <div class="form-group">
                            <label class="col-sm-2 control-label text-right">机柜名称: </label>
                            <div class="col-md-4 form-inner">
                                <input type="text" name="cupname" value="" class="col-sm-9">
                                <input type="hidden" name="id"/>
                                <span class="col-sm-3 red">必填</span>
                             </div>
                             <label class="col-sm-2 control-label text-right">机位数量: </label>
                            <div class="col-md-4 form-inner">
                                <input type="text" name="seatnum" value="16" onclick="this.value=''" class="col-sm-9">
                                <span class="col-sm-3 red">必填</span>
                             </div>
                        </div>
                        <div class="form-group">
                             <label class="col-sm-2 control-label text-right">机柜价格: </label>
                            <div class="col-md-4 form-inner">
                                <input type="text" name="price" value="0" class="col-sm-9">
                             </div>
                             <label class="col-sm-2 control-label text-right">所在机柜: </label>
                            <div class="col-md-4 form-inner">
                                <select  name="houid" class="col-sm-9">
                                    <option value></option>
                                    <?php if(is_array($house)): $i = 0; $__LIST__ = $house;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["houname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
...
$(function(){
    // 增加机房
    $("[data-target=addCup]").click(function(){
        var form = $("#createCup").get(0);
        form.action = "<?php echo U('Create/addCupboardHandle');?>";
        var url = location.search;
        var loc = url.substring(url.indexOf('?'),url.length).split('&');
        ...
        form.houid.value = houid
        $("#addCup").modal('show');
        $("#addCup").on('hide.bs.modal',function(){
            form.reset();
        });
    });
    // 删除机房
    $("[data-target=deleteHouse]").click(function(){
        ...
        if(confirm('确认要删除操作吗?')){
            $.post(url,{id:id},function(ret){
                $("#noticediv .modal-body").text(ret.data);
                $("#noticediv .modal-title").html(ret.info);
                $("#noticediv").modal('show');
                $("#noticediv").on('hide.bs.modal',function(){
                    if(ret.status == 1){
    ...
    $("[data-target=modifyhouse]").click(function(){
        var id = $(this).attr('cup');
        var form = $("#createCup").get(0);
        form.action = "<?php echo U('Public/amendCupHandle');?>";
        $.post("<?php echo U('Public/getCupInfo');?>",{id:id},function(ret){
            if(ret.status == 0){
                $("#noticediv .modal-title").text(ret.info);
                $("#noticediv .modal-body").html(ret.data);
                $("#noticediv").modal('show');
            }else{
                form.cupname.value = ret.data.cupname;
                form.seatnum.value = ret.data.seatnum;
                form.houid.value = ret.data.houid;
                form.id.value = id;
                form.price.value = ret.data.price;
                $("#addCup").modal('show');
                $("#addCup").on("hidden.bs.modal",function(){
                    form.reset();
                });
            }
    ...
    $("#createCup :submit").click(function(){
        var form = $("#createCup").get(0);
        var message = '';
        var state = true;
        if(form.cupname.value.length == 0){
            message += '机柜名称不能为空.<br>';
            state = false;
        }
        if(form.seatnum.value.length == 0){
            message += '机位不能为空.<br>';
            state = false;
        }
        if(form.houid.value.length == 0){
